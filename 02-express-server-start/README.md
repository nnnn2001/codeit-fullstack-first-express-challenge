...
- [x] 01-basic-setup이 완료되어 Express가 설치되어 있는지 확인
- [x] `package.json`에 `"type": "module"`이 설정되어 있는지 확인

### 2단계: Express 서버 파일 작성

- [x] `src/server.js` 파일을 다음 내용으로 수정:
...
- [x] **package.json 업데이트하기** - `scripts` 섹션의 `dev` 명령어를 다음과 같이 수정:
  ```json
...
- [x] 브라우저에서 `http://localhost:3000` 접속해서 JSON 응답 확인

- [x] 콘솔에 "🚀 Server running on http://localhost:3000" 메시지가 출력되는지 확인
...
- [x] 서버가 포트 3000에서 정상 실행되는가?
- [x] 브라우저에서 localhost:3000 접속 시 JSON 응답이 나타나는가?
- [x] 응답에 `message`와 `timestamp` 필드가 포함되어 있는가?
- [x] Postman에서 GET 요청이 성공하는가?
- [x] 서버 로그에 시작 메시지가 출력되는가?
